...
import { ArrowLeft, ShieldCheck, Truck, Headphones, CreditCard, Tags, Flame, Trophy } from "lucide-react";
...
import BrandStrip, { topBrandsFromProducts } from "@/components/commerce/BrandStrip";
...
export default async function Home() {
  ...
  const categories = categoryFile.data.filter((category) => category.active !== false);
  const inStock = products.filter((p) => p.stock > 0);
  const heroProducts = inStock.slice().sort(() => Math.random() - 0.5).slice(0, 6);
  ...
  // A couple of featured category rows so the homepage isn't just one long grid.
  const featuredCategoryNames = ["دریل و پیچ‌گوشتی", "لوازم ایمنی", "جوشکاری"].filter((name) =>
    categories.some((c) => c.name === name),
  );
  const categoryRows = featuredCategoryNames.map((name) => ({
    name,
    products: products.filter((p) => p.category === name && p.stock > 0).slice(0, 8),
  }));
      ...
      <main>
        ...
        <section className="mx-auto max-w-[1500px] px-4 py-8 lg:px-6">
          <div className="mb-7 flex items-end justify-between">
            <div>
              <div className="text-xs font-black text-[var(--primary)]">کاتالوگ</div>
              <h2 className="mt-1 text-2xl font-black">دسته‌بندی محصولات</h2>
              <p className="mt-2 text-sm text-[var(--muted)]">دسته‌ها مستقیماً از داشبورد مدیریت می‌شوند.</p>
            </div>
            <Link href="/products" className="hidden shrink-0 items-center gap-1 text-sm font-black text-[var(--primary)] sm:flex">
              همه محصولات <ArrowLeft size={15} />
            </Link>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {categories.slice(0, 8).map((category, index) => {
              ...
              return (
                <Link href={`/products?category=${encodeURIComponent(category.name)}`} key={category.id} className="card group p-5 hover:-translate-y-1 hover:border-[var(--primary)]">
                  <div className="flex items-center justify-between">
                    <div className="grid size-12 place-items-center rounded-2xl bg-[var(--primary)]/10 text-[var(--primary)]"><Tags size={20} /></div>
        ...
        <ProductRow title="جدیدترین محصولات" subtitle="تازه‌های فروشگاه" products={newest} viewAllHref="/products" />
