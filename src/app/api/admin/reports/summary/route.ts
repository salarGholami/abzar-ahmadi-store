...
export async function GET() {
  try {
    ...
    const [productsF, salesF, itemsF, financeF] = await Promise.all([
    ...
    const salesThisMonth = sales.filter((s) => new Date(s.createdAt).getTime() >= monthStart);

    const totalRevenue = sales.reduce((s, x) => s + Number(x.netAmount || 0), 0);
    const totalProfit = sales.reduce((s, x) => s + Number(x.grossProfit || 0), 0);
    ...
    const income = finance.filter((f) => ["SALE", "OTHER_INCOME"].includes(f.type)).reduce((s, f) => s + Number(f.amount || 0), 0);
    const expense = finance.filter((f) => ["PURCHASE", "OTHER_EXPENSE"].includes(f.type)).reduce((s, f) => s + Number(f.amount || 0), 0);

    return ok({
      totalRevenue, totalProfit, revenueThisMonth, salesCount: sales.length,
